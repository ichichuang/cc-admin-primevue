  ...
  setup(props) {
    ...
    const formatDateTimeForDisplay = (value: any): string => {
      ...
      if (value instanceof Date && !isNaN(value.getTime())) {
        return value.toISOString().slice(0, 16) // YYYY-MM-DDTHH:mm 格式
      }
    ...
    // 日期时间值解析函数 - 将字符串转换为 Date 对象
    const parseDateTimeFromString = (dateTimeString: string): Date | null => {
      if (!dateTimeString) {
        return null
      }
    ...
    const currentValue = ref(formatDateTimeForDisplay(props.params.value))
    const currentDateTimeValue = ref(
      parseDateTimeFromString(formatDateTimeForDisplay(props.params.value))
    )
    ...
    // AG Grid 编辑器必需的方法
    const getValue = () => {
      return currentDateTimeValue.value
    }
    ...
    const handleKeydown = (event: KeyboardEvent) => {
      ...
      if (key === 'Enter') {
        ...
        event.stopPropagation()
        // 重置到原始值
        currentValue.value = formatDateTimeForDisplay(props.params.value)
        currentDateTimeValue.value = parseDateTimeFromString(currentValue.value)
    ...
    // 日期时间选择事件处理
    const handleDateTimeSelect = (selectedDateTime: Date | null) => {
      currentDateTimeValue.value = selectedDateTime
      currentValue.value = formatDateTimeForDisplay(selectedDateTime)
    ...
    const handleInputChange = (value: string) => {
      ...
      // 尝试解析输入的日期时间字符串
      const parsedDateTime = parseDateTimeFromString(value)
      if (parsedDateTime) {
        currentDateTimeValue.value = parsedDateTime
      }
    }
